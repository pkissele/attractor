...
int main(int argc, char** argv) {
    ...
    thread simThread(step, ref(sim), ref(state), ref(running), dt, LOG_SIM_TIME);
    ...
    auto [axisVAO, axisVBO] = createStaticVAO(axisVerts, sizeof(axisVerts), {
        {0, 3, 6, 0},
        {1, 3, 6, 3}
    });

    float dummy[] = {0,0,0};
    auto [pointVAO, pointVBO] = createStaticVAO(dummy, sizeof(dummy), {
        {0, 3, 3, 0}
    });
    GLuint pointProgram = createProgram("shaders/point_vertex.glsl", "shaders/point_fragment.glsl");
    ...
    GLuint axisProgram = createProgram("shaders/axis_vertex.glsl", "shaders/axis_fragment.glsl");


    GLint colorLoc          = glGetUniformLocation(program,         "color");
    GLint mvpLoc            = glGetUniformLocation(program,           "mvp");
    GLint axisMvpLoc        = glGetUniformLocation(axisProgram,       "mvp");
    ...
    while (!glfwWindowShouldClose(window)) {
        if(!state.paused) curTime += elapsed/1000.0;
        ...
        Vec3f rot(cos(angle), 0.5, sin(angle));
        target = center;
        ...
        glUniformMatrix4fv(axisMvpLoc, 1, GL_FALSE, mvp.data());
        drawVAO(axisProgram, axisVAO, GL_LINES, 6);

        // Draw rotation center
        drawPoint(pointProgram, pointVAO, pointVBO, center, Vec3f(0, 1, 0), 10.0f, mvp);
...
void step(Attractor& engine, globalState& shared, atomic<bool>& running, double dt, bool LOG_SIM_TIME) {
    int step = 0;
    ...
    while (running) {
        ...
        bool done = engine.step(dt, step);
        auto end = chrono::high_resolution_clock::now();
        if(!done) {
            double elapsed = chrono::duration<double, milli>(end - start).count();

            Points& points = engine.getPoints();
...
void handleMoveKeys(GLFWwindow* w, globalState& s, float elapsed) {
    ...
    if (glfwGetKey(w, GLFW_KEY_W) == GLFW_PRESS)        (*s.center)[0] += 0.3f;
    if (glfwGetKey(w, GLFW_KEY_A) == GLFW_PRESS)        (*s.center)[2] -= 0.3f;
    if (glfwGetKey(w, GLFW_KEY_S) == GLFW_PRESS)        (*s.center)[0] -= 0.3f;
    if (glfwGetKey(w, GLFW_KEY_D) == GLFW_PRESS)        (*s.center)[2] += 0.3f;
}
